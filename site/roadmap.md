---
layout: page
title: v0.2 Roadmap
permalink: /roadmap/
---

The v0.2 program focuses on practical runtime capability expansion.

## Major Approved Candidates

1. I/O Foundation
2. File System Operations
3. Richer Standard Library Surface
4. Runtime Error and Exit Semantics

Reference: [v0.2 Feature Candidates](/reference/governance/v0.2/feature-candidates.md)

## Candidate Work Queue

Additional candidates queued for steering review:
- Path and Working Directory Utilities
- Deterministic Program Exit Contract
- Configuration and Environment Access
- Observability Helpers

## Ratification Signal

Candidates move to ratified scope when feasibility, governance rationale, and acceptance test expectations are all present.

See: [v0.2 User Stories](/reference/governance/v0.2/user-stories.md)
